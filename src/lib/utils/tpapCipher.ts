...
function hkdfExpand(label: string, prk: Buffer, length: number): Buffer {
  const salt = Buffer.alloc(length, 0);
  return Buffer.from(crypto.hkdfSync('sha256', prk, salt, label, length));
}

/** HKDF-derive for session keys */
function hkdfDerive(ikm: Buffer, salt: Buffer, info: Buffer, length: number): Buffer {
  return Buffer.from(crypto.hkdfSync('sha256', ikm, salt, info, length));
}

export interface TpapSessionKeys {
  key: Buffer;
  baseNonce: Buffer;
  stok: string;
  startSeq: number;
...
export default class TpapCipher {
  ...
  constructor(private readonly log: any, private readonly ip: string, private readonly email: string, private readonly password: string) {}
  ...
  async handshake(): Promise<void> {
    ...
    const authUsername = crypto.createHash('md5').update('admin').digest('hex');
    ...
    if (extraCrypt) {
      ...
      if (cryptType === 'password_shadow') {
        ...
        if (passwdId === 2) {
          credential = crypto.createHash('sha1').update(this.password).digest('hex');
    ...
    const derived = crypto.pbkdf2Sync(credentialBytes, devSalt, iterations, 80, 'sha256');
    ...
    const userConfirm = crypto.createHmac('sha256', keyConfirmA).update(rEncoded).digest();

    // Expected device confirm = HMAC-SHA256(keyConfirmB, lEncoded)
    const expectedDevConfirm = crypto.createHmac('sha256', keyConfirmB).update(lEncoded).digest();
  ...
  encrypt(payload: string): { data: Buffer; seq: number } {
    ...
    const cipher = crypto.createCipheriv('aes-128-ccm', this.key, nonce, { authTagLength: TAG_LEN });
  ...
  decrypt(data: Buffer): string {
    ...
    const decipher = crypto.createDecipheriv('aes-128-ccm', this.key, nonce, { authTagLength: TAG_LEN });
