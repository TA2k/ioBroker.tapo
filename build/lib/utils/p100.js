...
class P100 {
  constructor(log, ipAddress, email, password, timeout, port, useHttps) {
    ...
    if (this.useHttps) {
      this._axios = import_axios2.default.create({
    ...
    this.createKeyPair();
    this.terminalUUID = import_crypto.default.randomUUID();
    this._reconnect_counter = 0;
    this._timeout = timeout;
  }
  _crypto = import_crypto.default;
  _axios = import_axios2.default;
  _utf8 = import_utf8.default;
  is_klap = true;
  is_tpap = false;
  klap_version = 0;
  // 0 = unknown, 1 = v1 (md5), 2 = v2 (sha256)
  deviceMac = "";
  encodedPassword;
  encodedEmail;
  privateKey;
  ...
  getHttpsAgent() {
    if (!this.useHttps) return void 0;
    if (!this._httpsAgent) {
      this._httpsAgent = new import_https.default.Agent({ rejectUnauthorized: false });
    }
  ...
  calc_auth_hash(username, password) {
    ...
    return this._crypto.createHash("sha256").update(Buffer.concat([usernameDigest, passwordDigest])).digest();
  }
  calc_auth_hash_v1(username, password) {
    ...
    const usernameMd5 = this._crypto.createHash("md5").update(Buffer.from(username.normalize("NFKC"))).digest();
    const passwordMd5 = this._crypto.createHash("md5").update(Buffer.from(password.normalize("NFKC"))).digest();
    return this._crypto.createHash("md5").update(Buffer.concat([usernameMd5, passwordMd5])).digest();
  }
  createKeyPair() {
  ...
  async raw_request(path, data, responseType, params) {
    ...
    return this._axios.post(URL, data, config).then((res) => {
      ...
      var _a;
      this.log.debug("276 Error: " + ((error == null ? void 0 : error.message) || error) + " " + this.ip);
      if (((_a = error == null ? void 0 : error.message) == null ? void 0 : _a.indexOf("403")) > -1) {
        this.reAuthenticate();
      }
      return false;
    });
  ...
  async handshake_new() {
    ...
    const options = {
      ...
      agent: this.useHttps ? new import_https.default.Agent({ keepAlive: true, rejectUnauthorized: false }) : new import_http.default.Agent({ keepAlive: true })
    };
    this.log.debug("KLAP handshake1 " + (this.useHttps ? "https" : "http") + "://" + this.ip + ":" + this.port + "/app/handshake1");
    const response = await new Promise((resolve, reject) => {
      const requester = this.useHttps ? import_https.default : import_http.default;
      const request = requester.request(options, (res) => {
        const chunks = [];
        if (res.headers && res.headers["set-cookie"]) {
          this.cookie = res.headers["set-cookie"][0].split(";")[0];
        }
    ...
    const calcSeedHash = (ah, version) => {
      const payload = version === 1 ? Buffer.concat([local_seed, ah]) : Buffer.concat([local_seed, remote_seed, ah]);
    ...
    const handshake2Payload = matchedVersion === 1 ? Buffer.concat([remote_seed, auth_hash]) : Buffer.concat([remote_seed, local_seed, auth_hash]);
    const req = this._crypto.createHash("sha256").update(handshake2Payload).digest();
  ...
  async handshake_tpap() {
    ...
    this.tpapCipher = new import_tpapCipher.default(this.log, this.ip, this.email, this.password, this.deviceMac, this.port, this.useHttps);
    let pakeList = [2];
    let userHashType = 0;
    try {
      ...
      }, { timeout: 5e3, httpsAgent: this.getHttpsAgent() });
      const tpap = (_b = (_a = discoverRes.data) == null ? void 0 : _a.result) == null ? void 0 : _b.tpap;
      if (tpap == null ? void 0 : tpap.pake) {
        pakeList = tpap.pake;
        if (tpap.mac) {
    ...
    if (this.tpLinkCipher) {
      ...
      return this._axios.post(URL, securePassthroughPayload, config).then((res) => {
        ...
        try {
          ...
          this.log.debug((error == null ? void 0 : error.stack) || error);
          return this.handleError(JSON.parse(decryptedResponse).error_code, "340");
        }
      }).catch((error) => {
        this.log.debug("371 Error: " + ((error == null ? void 0 : error.message) || error));
        return error;
      });
    } else if (this.newTpLinkCipher) {
      const data = this.newTpLinkCipher.encrypt(payload);
      const URL2 = this.getBaseUrl() + "/app/request";
      this.log.debug("KLAP getDeviceInfo URL: " + URL2);
      const headers2 = {
        Connection: "Keep-Alive",
        Host: this.ip,
        Accept: "*/*",
      ...
      return this._axios.post(URL2, data.encryptedPayload, config).then((res) => {
        ...
        try {
          ...
          return this.handleError(((_a = res.data) == null ? void 0 : _a.error_code) || error.message, "480");
        }
      }).catch((error) => {
        var _a;
        this.log.debug("469 Error: " + ((error == null ? void 0 : error.message) || error));
        if (((_a = error == null ? void 0 : error.message) == null ? void 0 : _a.indexOf("403")) > -1) {
          this.reAuthenticate();
        }
        return error;
      ...
      return this.handleTpapRequest(payload).then((response) => {
        if (!response || response.error_code !== void 0 && response.error_code !== 0) {
          return this.handleError((response == null ? void 0 : response.error_code) || "unknown", "tpap_getDeviceInfo");
        }
        this.setSysInfo(response.result);
        this.log.debug("Device Info: ", response.result);
        return this.getSysInfo();
        ...
        const needsReconnect = status === 401 || status === 403 || code === "ECONNABORTED" || code === "ETIMEDOUT" || code === "ECONNREFUSED" || code === "ECONNRESET";
        if (needsReconnect && this._reconnect_counter <= 3) {
          ...
          try {
            ...
            if (!response || response.error_code !== void 0 && response.error_code !== 0) {
              return this.handleError((response == null ? void 0 : response.error_code) || "unknown", "tpap_getDeviceInfo_retry");
            }
            this._reconnect_counter = 0;
            this.setSysInfo(response.result);
            return this.getSysInfo();
      ...
      if (this.newTpLinkCipher) {
        ...
        return this.raw_request("request", data.encryptedPayload, "arraybuffer", { seq: data.seq.toString() }).then((res) => {
          ...
          if (!res || !Buffer.isBuffer(res)) {
            const responseData = (_a = res == null ? void 0 : res.response) == null ? void 0 : _a.data;
  ...
  async sendCommand(method, params) {
    ...
    const handler = this.tpapCipher && this.tpapCipher.isReady ? () => this.handleTpapRequest(payload) : this.tpLinkCipher ? () => this.handleRequest(payload) : () => this.handleKlapRequest(payload);
    const doReconnect = this.tpapCipher && this.tpapCipher.isReady ? () => this.tpapReconnect() : this.tpLinkCipher ? () => this.reconnect() : () => this.newReconnect();
    try {
      const response = await handler();
      this._reconnect_counter = 0;
      return (_a = response == null ? void 0 : response.result) != null ? _a : response;
    } catch (error) {
      const code = error.code;
      const shouldReconnect = ((_b = error.message) == null ? void 0 : _b.includes("9999")) || ((_c = error.response) == null ? void 0 : _c.status) === 401 || ((_d = error.response) == null ? void 0 : _d.status) === 403 || ((_e = error.message) == null ? void 0 : _e.includes("TPAP cipher not ready")) || code === "ECONNABORTED" || code === "ETIMEDOUT" || code === "ECONNREFUSED" || code === "ECONNRESET";
      if (shouldReconnect && this._reconnect_counter <= 3) {
        this._reconnect_counter++;
        await doReconnect();
        const response = await handler();
        this._reconnect_counter = 0;
        return (_f = response == null ? void 0 : response.result) != null ? _f : response;
      }
      this._reconnect_counter = 0;
      throw error;
    }
  ...
  async reAuthenticate() {
    ...
    try {
      ...
      this.log.debug("Reauthenticate failed " + this.ip + ": " + ((error == null ? void 0 : error.message) || error || "unknown"));
      throw error;
    }
  }
}
