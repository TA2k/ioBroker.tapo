    ...
    await this.requestClient({
      ...
      for (const device of ((_b = res.data.result) == null ? void 0 : _b.deviceList) || []) {
        ...
        const cameraRemotes = [
          ...
          { command: "setLineCrossingDetection", name: "Line Crossing Detection On/Off" },
    ...
    if (device.deviceName === "P100") {
      ...
      if (this.deviceObjects[id].getPresets) {
        try {
          ...
          this.log.debug(`getPresets failed for ${id}: ${(e == null ? void 0 : e.message) || e}`);
        }
      }
      return;
      ...
      if (!state.ack) {
        ...
        try {
          const cameraCommands = {
            ...
            setLineCrossingDetection: "lineCrossingDetection",
          ...
          if (this.deviceObjects[deviceId] && (this.deviceObjects[deviceId][command] || cameraCommands[command])) {
            ...
            if ((command === "savePreset" || command === "deletePreset") && this.deviceObjects[deviceId].getPresets) {
              try {
                ...
                this.log.debug(`getPresets refresh failed for ${deviceId}: ${(e == null ? void 0 : e.message) || e}`);
              }
            }
            this.refreshTimeout && clearTimeout(this.refreshTimeout);
        ...
        const resultDict = {
          ...
          lineCrossingDetection: "setLineCrossingDetection",
